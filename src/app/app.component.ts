...
export class AppComponent {
  ...
  onRowUpdated({ data }) {
    const changedId = data.EmployeeID;
    const filteredRecord = this.tasksDataSourceStorage.filter(
      data => data.key === changedId
    );
    const filteredItems = filteredRecord.length
      ? filteredRecord[0].dataSourceInstance.items()
      : [];
    const updatedTotal = filteredItems.reduce((acc, current) => {
      acc = acc + current.amount;
      return acc;
    }, 0);
    this.employees.forEach(item => {
      if (item.ID === changedId) {
        item.total = updatedTotal;
      }
    });
  }
}
